...
export async function handleWebSocketUpgrade(request, env) {
    try {
        ...
        const rpcResponse = newWorkersRpcResponse(server, pdfApi);
        // Track connection
        const connection = {
            socket: server,
            sessionId,
        ...
        console.log(`[WebSocket] Connection established: sessionId=${sessionId}, userId=${userId}, active=${activeConnections.size}`);
...
function setupHeartbeat(connection) {
    ...
    const interval = setInterval(() => {
        ...
        if (timeSinceLastHeartbeat > HEARTBEAT_TIMEOUT) {
            console.warn(`[WebSocket ${sessionId}] Heartbeat timeout (${timeSinceLastHeartbeat}ms), closing connection`);
 ...
 */
function handleDisconnect(sessionId, code, reason) {
    const connection = activeConnections.get(sessionId);
    if (!connection) {
        console.warn(`[WebSocket] Disconnect for unknown session: ${sessionId}`);
        return;
    }
    console.log(`[WebSocket ${sessionId}] Disconnected: code=${code}, reason="${reason}", active=${activeConnections.size - 1}`);
 ...
 */
export function closeSession(sessionId, reason = 'Admin close') {
    const connection = activeConnections.get(sessionId);
    if (!connection) {
        return false;
    }
