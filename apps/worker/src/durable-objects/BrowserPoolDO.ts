...
export class BrowserPoolDO {
  ...
  async releaseBrowser(browser: Browser): Promise<void> {
    ...
    if (instance.pdfsGenerated >= 1000 || browserAge >= oneHour) {
      console.log(
        `Browser ${instance.instanceId} needs recycling (PDFs: ${instance.pdfsGenerated}, Age: ${Math.round(browserAge / 1000 / 60)}min)`
      );
  ...
  async cleanup(): Promise<void> {
    ...
    for (const instance of idleInstances) {
      ...
      console.log(
        `Cleaning up browser ${instance.instanceId} (idle for ${idleMinutes} minutes)`
      );
      await this.recycleBrowser(instance.instanceId);
    }

    console.log(
      `Cleanup complete. Remaining browsers: ${this.browserPoolState.browserInstances.length}`
    );
  }
}
