...
export class BrowserPoolDO {
  ...
  private env: BrowserPoolEnv;
  private browserPoolState: BrowserPoolState;
  ...
  private cleanupIntervalHandle?: number;

  constructor(state: DurableObjectState, env: BrowserPoolEnv) {
    this.state = state;
    this.env = env;
   ...
   */
  private async handleGeneratePdf(request: Request): Promise<Response> {
  ...
  private async handleStats(): Promise<Response> {
    ...
    const reuseRate =
      this.sessionMetrics.totalBrowserAcquisitions > 0
    ...
    const stats = {
      ...
      poolCapacity: {
        ...
        utilizationPercent: Math.round(
          (this.browserPoolState.currentLoad / BROWSER_POOL_CONFIG.maxPoolSize) * 100
        ),
      },
    };
  ...
  async acquireBrowser(): Promise<Browser> {
    ...
    const idleBrowser = this.browserPoolState.browserInstances.find(
      (b) => b.status === 'idle' && b.browserHandle
    );
    ...
    while (Date.now() - startWait < maxWaitTime) {
      ...
      const nowIdleBrowser = this.browserPoolState.browserInstances.find(
        (b) => b.status === 'idle' && b.browserHandle
      );
    ...
    const oldestBrowser = this.browserPoolState.browserInstances.sort(
  ...
  async releaseBrowser(browser: Browser): Promise<void> {
    ...
    if (instance.pdfsGenerated >= 1000 || browserAge >= maxAge) {
      console.log(
        `[Pool] Browser ${instance.instanceId} needs recycling (PDFs: ${instance.pdfsGenerated}, Age: ${Math.round(browserAge / 1000 / 60)}min)`
      );
      await this.recycleBrowser(instance.instanceId);
      return;
    }
   ...
   */
  private async checkBrowserHealth(browser: Browser, instanceId: string): Promise<boolean> {
    try {
   ...
   */
  async recycleBrowser(instanceId: string): Promise<void> {
    const instance = this.browserPoolState.browserInstances.find(
    ...
    // Remove from pool
    this.browserPoolState.browserInstances = this.browserPoolState.browserInstances.filter(
      (b) => b.instanceId !== instanceId
    );

    console.log(
      `Recycled browser ${instanceId}. Pool size: ${this.browserPoolState.browserInstances.length}`
    );
  }
  ...
  async cleanup(): Promise<void> {
    ...
    for (const instance of sortedByAge) {
      ...
      console.log(
        `[Pool] FIFO evict: ${instance.instanceId} (idle: ${idleMinutes}min, age: ${ageMinutes}min, pdfs: ${instance.pdfsGenerated})`
      );
    ...
    console.log(
      `[Pool] Cleanup complete. Pool size: ${this.browserPoolState.browserInstances.length} (reuse rate: ${Math.round((this.sessionMetrics.warmBrowserHits / Math.max(1, this.sessionMetrics.totalBrowserAcquisitions)) * 100)}%)`
    );
  }
}
