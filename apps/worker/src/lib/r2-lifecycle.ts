...
export interface R2LifecycleRule {
  ...
  /** Object prefix filter (e.g., "pdfs/free/") */
  prefix?: string;

  /** Tag filter (e.g., { tier: "free" }) */
  tags?: Record<string, string>;

  /** Days after which objects are deleted */
 ...
 * - Enterprise: 90 days
 */
export const R2_LIFECYCLE_RULES: R2LifecycleRule[] = [
  {
 ...
 */
export function getLifecycleRuleForTier(tierId: string): R2LifecycleRule | null {
  return (
    R2_LIFECYCLE_RULES.find((rule) => rule.tags?.tier === tierId.toLowerCase()) || null
  );
}
 ...
 */
export function calculateExpirationDate(tierId: string, uploadDate: Date = new Date()): Date {
 ...
 */
export function isPdfExpired(tierId: string, uploadDate: Date): boolean {
 ...
 */
export function getExpirationMessage(tierId: string): string {
  const retentionDays = getRetentionDays(tierId);
