 ...
 */
function getRetentionDaysForTier(tier) {
    const retentionMap = {
        free: 1,
        starter: 7,
        pro: 30,
 ...
 * ```
 */
export async function uploadPdfToR2(options) {
 ...
 * ```
 */
export async function deletePdfFromR2(bucket, fileName) {
    try {
 ...
 * ```
 */
export async function pdfExistsInR2(bucket, fileName) {
    try {
 ...
 * ```
 */
export async function getPdfMetadata(bucket, fileName) {
    try {
 ...
 * ```
 */
export async function listExpiredPdfs(bucket, limit = 1000) {
    const expiredFiles = [];
    const now = new Date();
    try {
 ...
 * ```
 */
export function getPdfPublicUrl(fileName, customDomain = 'cdn.speedstein.com') {
