...
import type { PdfOptions, PdfMetadata } from '@speedstein/shared/types/pdf';
import {
  DEFAULT_PDF_OPTIONS,
  MAX_HTML_SIZE_BYTES,
  PDF_GENERATION_TIMEOUT_MS,
} from '@speedstein/shared/types/pdf';
import {
  BrowserError,
...
/**
 * PDF generation result (internal)
 */
interface PdfGenerationResult {
  /** The generated PDF as ArrayBuffer */
...
export class PdfService {
   ...
   * @private
   */
  private async generatePdfWithPage(
    page: any,
    html: string,
    options: PuppeteerPdfOptions
  ): Promise<ArrayBuffer> {
   ...
   */
  parseOptions(userOptions: PdfOptions | undefined): Required<PdfOptions> {
    return {
      ...
      displayHeaderFooter:
        userOptions?.displayHeaderFooter ?? DEFAULT_PDF_OPTIONS.displayHeaderFooter,
   ...
   */
  convertToPuppeteerOptions(options: PdfOptions): PuppeteerPdfOptions {
   ...
   * @throws PayloadTooLargeError if HTML exceeds size limit
   */
  validateHtmlSize(html: string): void {
   ...
   */
  calculateEstimatedSize(html: string): number {
   ...
   */
  sanitizeOptions(options: any): PdfOptions {
    const {
      path, // Remove - we use in-memory buffers only
   ...
   */
  getDefaultOptions(): Required<Omit<PdfOptions, 'margin' | 'headerTemplate' | 'footerTemplate'>> {
