...
import type { SubscriptionTier } from '@speedstein/shared/types/user'
...
export interface R2UploadResult {
  ...
  /** Expiration timestamp based on tier */
  expiresAt: string
...
export class R2Service {
   ...
   */
  async getPdfMetadata(key: string): Promise<Record<string, string> | null> {
   ...
   */
  async listUserPdfs(userId: string, limit: number = 100): Promise<string[]> {
    const prefix = `${userId}/`
   ...
   * @param userId - User ID
   */
  async deleteAllUserPdfs(userId: string): Promise<number> {
   ...
   */
  async cleanupExpiredPdfs(batchSize: number = 100): Promise<number> {
    let deletedCount = 0
 ...
 */
export function extractR2KeyFromUrl(url: string, publicDomain: string): string | null {
