...
import { validatePdfGenerationRequest, } from '../lib/validation';
import { getDOStubForUser } from '../lib/browser-pool-manager';
export class PdfGeneratorApi extends RpcTarget {
    userId;
    browserPoolNamespace;
    sessionId;
    requestCount = 0;
    sessionMetadata;
    constructor(userId, browserPoolNamespace, sessionId, connectionType = 'websocket') {
        super();
        this.userId = userId;
    ...
    async generatePdf(html, options) {
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`[RPC ${this.sessionId}] PDF generation failed: ${result.error || 'Unknown error'}`);
                return {
                    success: false,
            ...
            console.log(`[RPC ${this.sessionId}] PDF generated successfully in ${result.generationTime}ms (total: ${generationTime}ms)`);
            return {
                success: true,
     ...
     */
    async generateBatch(jobs) {
        const startTime = Date.now();
        this.requestCount += jobs.length;
        this.sessionMetadata.requestCount += jobs.length;
        this.sessionMetadata.lastActivityAt = new Date();
        try {
            console.log(`[RPC ${this.sessionId}] generateBatch request with ${jobs.length} jobs for user ${this.userId}`);
            // Validate jobs array
            ...
            const resultsPromises = jobs.map((job) => this.generatePdf(job.html, job.options).then((result) => ({
                ...result,
                fileName: job.id,
            })));
            ...
            console.log(`[RPC ${this.sessionId}] Batch complete: ${successfulJobs}/${jobs.length} successful in ${totalTime}ms (${Math.round((jobs.length / totalTime) * 60000)} PDFs/min)`);
            return {
                success: true,
                results,
    ...
    [Symbol.dispose]() {
        ...
        this.sessionMetadata.lastActivityAt = new Date();
        console.log(`[RPC ${this.sessionId}] Disposing PdfGeneratorApi for user ${this.userId} (${this.requestCount} requests, duration: ${Date.now() - this.sessionMetadata.createdAt.getTime()}ms)`);
