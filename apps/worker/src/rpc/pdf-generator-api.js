...
import { generatePdfFileName } from '../lib/r2';
/**
 * PDF Generator RPC API
 *
...
export class PdfGeneratorApi extends RpcTarget {
    ...
    constructor(userId, browserPoolDO, sessionId) {
        super();
        this.userId = userId;
    ...
    async generatePdf(request) {
        ...
        try {
            ...
            return this.createErrorResponse('GENERATION_ERROR', error instanceof Error ? error.message : 'PDF generation failed');
        }
    }
    /**
    ...
    async generateBatch(request) {
        ...
        try {
            ...
            if (request.pdfs.length > RPC_CONFIG.maxBatchSize) {
                return this.createErrorResponse('VALIDATION_ERROR', `Batch size exceeds maximum (${RPC_CONFIG.maxBatchSize})`);
            }
            const startTime = Date.now();
            ...
            return this.createErrorResponse('BATCH_ERROR', error instanceof Error ? error.message : 'Batch generation failed');
        }
    }
    /**
     ...
     * @private
     */
    async generatePdfInternal(html, options) {
